...
/**
 * 自动绑定（来自jfinal ext) 1.如果没用加入注解，必须以Controller结尾,自动截取前半部分为key 2.加入TableBind的 获取
 * key
 * 
 * @author zb 2014-3-20
 */
public class AutoBindModels {
	...
	public void config() {
		...
		for (Class model : controllerClasses) {
			...
			tableBind = (TableBind) model.getAnnotation(TableBind.class);
			if (tableBind == null || StrKit.isBlank(tableBind.tableName())) {
				continue;
			}
			
			// all default ,so not null
			if (StrKit.isBlank(tableBind.configName()) || StrKit.isBlank(configName)) {
				logger.error("routes.add is null");
				continue;
			}

			// join many database support
			if (configName.equals(tableBind.configName())) {
				arp.addMapping(tableBind.tableName(), tableBind.pkName(), model);
				logger.debug(tableBind.configName() //
						+ " --> routes.add(" + model + "," + tableBind.tableName() + ", " + tableBind.pkName() + ")");
			}
